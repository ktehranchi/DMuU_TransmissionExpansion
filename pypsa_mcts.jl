...
pypsa_MDP = QuickPOMDP(
    ...
    TR = function (state, action)
        ...
        def WECC_simulator(state, action):
            network, state= update_network(network, state, action)
            sim_year = state[0]
        ...
        hvac_cost_dist = Uniform(hvdc_cost[1],hvdc_cost[2])
        ...
        annualized_cost = annualize(random_cost_to_build) 

        operating_cost, new_state = py"WECC_simulator"(state,action)

        annual_reward = operating_cost + annualized_cost

        return (sp=new_state, r=annual_reward)
